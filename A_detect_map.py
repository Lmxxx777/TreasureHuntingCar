...
def find_locating_box(frame, display=False):
    ...
    if display:
        ...
        cv2.putText(contours_frame, "counts:{}".format(len(contours)), (10, 20), cv2.FONT_HERSHEY_SIMPLEX, 0.5,
                    (0, 255, 0), 1)
        ...
        "FPS: {:.2f}".format(fps)
        cv2.putText(contours2_frame, "min_area:{},max_area:{},counts:{}".format(min_area, max_area, len(contours2)),
        ...
        cv2.putText(sibianxing_frame,
                    "approxPolyDP_epslion:{:.2f},counts:{}".format(approxPolyDP_epslion, len(sibianxing)), (10, 20),
        ...
        cv2.putText(zhengfangxing_frame, "wh_rate:{:.2f},counts:{}".format(wh_rate, len(zhengfangxing)), (10, 20),
                    cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 0), 1)
...
def treasure_Identification1(img, display=False):
    ...
    warped_img = img.copy()
    gray = cv2.cvtColor(warped_img, cv2.COLOR_BGR2GRAY)
    circles = cv2.HoughCircles(gray, cv2.HOUGH_GRADIENT, dp=1, minDist=minDist, param1=param1, param2=param2,
                               minRadius=min_ridus, maxRadius=max_ridus)
    centers0 = []
    if circles is not None:
        ...
        for (x, y, r) in circles:
            if 150 < x < 650 and 150 < y < 650:
                ...
                centers0.append((x, y))
    if display:
        cv2.putText(warped_img,
                    "minDist,param1,param2,min_ridus,max_ridus={},{},{},{},{}".format(minDist, param1, param2,
                                                                                      min_ridus, max_ridus), (10, 20),
                    cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 0), 1)
...
def get_maze_map_pose(img, display=False):
    ...
    if len(locating_points) == 4:
        ...
        if warped_img is not None:
            ...
            # print("宝藏位置-pixel", maze_piexl_location)
            # print("宝藏位置-point", map_maze_loaction)
            ...
            my_str = []
            for x in range(len(map_maze_loaction)):
                my_str.append('(' + str(map_maze_loaction[x][0]) + ', ' + str(map_maze_loaction[x][1]) + ')')
            idx = 0
            for pt in origin_maze_pixel:
                center = (pt[0], pt[1])
                print(my_str[idx])
                cv2.putText(frame,my_str[idx],center, cv2.FONT_HERSHEY_SIMPLEX,1.0,(255,0,0),2)
                idx += 1

            print(map_maze_loaction)
...
if __name__ == "__main__":
    ...
    while True:
        ...
        if ret:
            ...
            cv2.putText(img, "FPS: {:.2f},Success_rate:{:.2f}".format(fps, success_rate), (10, 20),
                        cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 0), 1)
